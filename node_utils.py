...
def fill_domain(boundary_nodes, domain_conditions, num_domain_nodes):
    """
    ...
    x_potentials = np.linspace(x_min, x_max, 102)[1:-1]
    y_potentials = np.linspace(y_min, y_max, 102)[1:-1]
...
def make_rectangle(x_min, x_max, y_min, y_max, num_ns_nodes, num_ew_nodes, condition_dict, value_dict, deriv_dict):
    """
    ...
    # top of rectangle
    nodes = np.concatenate((nodes, np.linspace(x_min, x_max, num_ns_nodes+1)[:-1] + y_max * 1j))

    # right edge
    nodes = np.concatenate((nodes, x_max + np.linspace(y_max, y_min, num_ew_nodes+1)[:-1] * 1j))

    # bottom of rectangle
    nodes = np.concatenate((nodes, np.linspace(x_max, x_min, num_ns_nodes+1)[:-1] + y_min * 1j))

    # left edge
    nodes = np.concatenate((nodes, x_min + np.linspace(y_min, y_max, num_ew_nodes+1)[:-1] * 1j))
    ...
    labels = fill_array(condition_dict)
    boundary_vals = fill_array(value_dict)
    deriv_lambdas = fill_array(deriv_dict)

    return nodes, labels, boundary_vals, deriv_lambdas


def make_circle(centre, radius, num_nodes, condition, value, deriv):
    """
    ...
    nodes = centre + radius * np.cos(thetas) + 1j * radius * np.sin(thetas)

    return nodes, np.full(num_nodes, condition), np.full(num_nodes, value), np.full(num_nodes, deriv)
